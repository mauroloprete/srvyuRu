...
#' @name tidyeval
#' @keywords internal
#' @importFrom rlang enquo enquos .data := as_name as_label sym parse_expr eval_tidy
#' @aliases enquo enquos .data := as_name as_label sym parse_expr eval_tidy
#' @export enquo enquos .data := as_name as_label sym parse_expr eval_tidy
NULL


glue_eval <- function(expr) {
  expr %>%
    parse_expr() %>%
    eval_tidy()
}
