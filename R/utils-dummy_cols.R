...
dummy_cols <- function(.data,
  ...
  for (col_name in char_cols) {
    ...
    .data[, paste0("dummy_", unique_vals, "_", col_name)] <- 0L
    for (unique_value in unique_vals) {
      data.table::set(.data,
        ...
        j = paste0("dummy_", unique_value, "_", col_name), value = 1L
      )
      ...
      if (!is.na(unique_value)) {
        data.table::set(.data,
          ...
          j = paste0("dummy_", unique_value, "_", col_name), value = NA
        )
      }
      ...
      if (!is.null(split)) {
        ...
        if (is.na(unique_value)) {
          .data[[paste0("dummy_", unique_value, "_", col_name)]][which(!is.na(.data[[col_name]]))] <- 0
        }
      }
    }
  }
    ...
    if (length(select_columns) == 1) {
      new_col_index <-
        as.logical(rowSums(sapply(unique_vals, function(x) {
          grepl(paste0("dummy_", select_columns, "_", x), names(.data))
        })))
      names(.data)[new_col_index] <-
        gsub(paste0("dummy_", select_columns, "_"), "", names(.data)[new_col_index])
    } else {
